...
public class PhysicalDbGroup {
    ...
    public PhysicalDbInstance select(Boolean master) {
        ...
        List<PhysicalDbInstance> instances = getRWDbInstances(master == null || master);
        if (instances.size() == 0) {
            return writeDbInstance;
        }
    ...
    private List<PhysicalDbInstance> getRWDbInstances(boolean includeWrite) {
        ...
        for (PhysicalDbInstance ds : allSourceMap.values()) {
            if (includeWrite && rwSplitMode == RW_SPLIT_ALL && ds == writeDbInstance && writeDbInstance.isAlive()) {
                okSources.add(ds);
                continue;
            }
