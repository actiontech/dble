...
public class ERJoinChooser {
    ...
    private JoinNode innerJoinOptimizer() {
        ...
        for (JoinRelationGroup group : joinRelationGroups) {
            JoinNode erJoinNode = tryMakeERJoin(group.getSelList());
            if (erJoinNode != null) {
    ...
    private JoinNode makeERJoin(List<JoinColumnInfo> erKeys) {
        ...
        for (int index = 1; index < erKeys.size(); index++) {
            ...
            if (t1 instanceof JoinNode) {
                ...
                t0 = toMakeERJoin(t0, t1, true);
            }
        }
        return (JoinNode) t0;
    }

    private JoinNode toMakeERJoin(PlanNode t0, PlanNode t1, boolean isReplace) {
        JoinNode newJoinNode = new JoinNode(t0, t1);
        List<ItemFuncEqual> joinFilter = toMakeJoinFilters(newJoinNode, t0, t1, true);
    ...
    private PlanNode makeJoinNode(List<PlanNode> units) {
        ...
        for (int i = 1; i < units.size(); i++) {
            ret = toMakeERJoin(ret, units.get(i), true);
        }
        return ret;
    }
     ...
     */
    private List<ItemFuncEqual> toMakeJoinFilters(JoinNode newJoinNode, PlanNode t0, PlanNode t1, boolean isReplace) {
        ...
        for (JoinRelationGroup group : joinRelationGroups) {
            ...
            if (filters.isEmpty() && !tempOnList.isEmpty()) {
                ...
                if (col0 != null && col1 != null && isErRelation(col0.getErTable(), col1.getErTable())) {
                    ItemFuncEqual bf = FilterUtils.equal(col0.key, col1.key);
                    filters.add(bf);
    ...
    private PlanNode joinWithGlobal(PlanNode t, List<PlanNode> globalList) {
        ...
        while (globalList.size() > 0) {
            ...
            for (int i = 0; i < globalList.size(); i++) {
                ...
                List<ItemFuncEqual> jnFilter = toMakeJoinFilters(joinNode, newT, global, false);
