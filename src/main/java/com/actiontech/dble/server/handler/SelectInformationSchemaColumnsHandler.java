...
public class SelectInformationSchemaColumnsHandler {
    ...
    public void sqlhandle(ShardingService service, FieldPacket[] fields0, MySqlSelectQueryBlock mySqlSelectQueryBlock) {
        ...
        if ((cSchema = containsKeyIngoreCase(whereInfo, INFORMATION_SCHEMACOLUMNS_COLS[1])) == null || (table = containsKeyIngoreCase(whereInfo, INFORMATION_SCHEMACOLUMNS_COLS[2])) == null) {
            MysqlSystemSchemaHandler.doWrite(fields0.length, fields0, null, service);
            return;
        }
        ...
        ShardingUserConfig userConfig = (ShardingUserConfig) (DbleServer.getInstance().getConfig().getUsers().get(service.getUser()));
        ...
        if (result != null) {
            ...
            for (Map<String, String> data : result) {
                ...
                for (String col : selectCols) {
                    row.add(null == data.get(col) ? null : StringUtil.encode(data.get(col), service.getCharset().getResults()));
                }
                rows[index++] = row;
            }
        }
        MysqlSystemSchemaHandler.doWrite(fieldCount, fields, rows, service);
    }
