...
public final class AlertGeneralConfig {
    ...
    private static final String CONFIG_FILE_NAME = "/OBsharding-D_alert.properties";
    ...
    public Alert customizedAlert() {
        if (properties != null && properties.getProperty(CONFIG_ALERT) != null) {
            ...
            try {
                ...
                LOGGER.info("User customized Alert error,OBsharding-D will use default Alert", e);
            }
        } else {
            LOGGER.info("No alert config in conf dir ,start with OBsharding-D self alert");
        }
