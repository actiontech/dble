...
public class PhysicalDbGroup {
    ...
    private void reportHeartbeatError(PhysicalDbInstance ins) throws IOException {
        ...
        if (dbGroupConfig.isShowSlaveSql()) {
            heartbeatError += " and the privileges of user is sufficient (NOTE:heartbeat[show slave status] need grant the SUPER or REPLICATION CLIENT privilege(s) to db user,and then restart the OBsharding-D or fresh conn).";
        }
