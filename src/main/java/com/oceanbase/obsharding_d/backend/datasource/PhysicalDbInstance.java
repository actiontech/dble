...
import com.oceanbase.obsharding_d.backend.delyDetection.DelayDetectionStatus;
import com.oceanbase.obsharding_d.backend.heartbeat.MySQLHeartbeat;
import com.oceanbase.obsharding_d.backend.mysql.nio.handler.ConnectionHeartBeatHandler;
...
import com.oceanbase.obsharding_d.net.factory.MySQLConnectionFactory;
...
import static com.oceanbase.obsharding_d.backend.datasource.PhysicalDbGroup.RW_SPLIT_OFF;
...
public abstract class PhysicalDbInstance implements ReadTimeStatusInstance {
    ...
    protected void checkPoolSize() {
        ...
        if (size < initSize) {
            LOGGER.warn("For db instance[{}], minIdle is less than (the count of shardingNodes), so OBsharding-D will create at least 1 conn for every schema, " +
