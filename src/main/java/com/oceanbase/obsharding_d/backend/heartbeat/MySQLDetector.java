...
public class MySQLDetector implements SQLQueryResultListener<SQLQueryResult<Map<String, String>>> {
    ...
    private boolean checkRecoverFail(PhysicalDbInstance source) {
        ...
        if (heartbeat.getStatus() == MySQLHeartbeatStatus.OK) { // ok->ok
            ...
            if (variables == null ||
                    variables.isLowerCase() != OBsharding_DServer.getInstance().getSystemVariables().isLowerCaseTableNames() ||
                ...
                } else if (variables.isLowerCase() != OBsharding_DServer.getInstance().getSystemVariables().isLowerCaseTableNames()) {
                    errMsg = "this dbInstance[=" + url + "]'s lower_case is wrong";
                } else if (versionMismatch) {
                    if (!source.getDbGroup().isRwSplitUseless()) {
                        //rw-split
                        errMsg = "the OBsharding-D version[=" + SystemConfig.getInstance().getFakeMySQLVersion() + "] and " + source.getDbGroupConfig().instanceDatabaseType().toString() + "[" + source.getConfig().getUrl() + "] version[=" + variables.getVersion() + "] not match, Please check the version.";
                    } else {
                        errMsg = "this dbInstance[=" + url + "]'s version[=" + variables.getVersion() + "] cannot be lower than the OBsharding-D version[=" + SystemConfig.getInstance().getFakeMySQLVersion() + "],pls check the backend " + source.getDbGroupConfig().instanceDatabaseType() + " node.";
                    }
                } else {
                    errMsg = "this dbInstance[=" + url + "]'s max_allowed_packet is " + variables.getMaxPacketSize() + ", but OBsharding-D's is " + SystemConfig.getInstance().getMaxPacketSize();
                }
