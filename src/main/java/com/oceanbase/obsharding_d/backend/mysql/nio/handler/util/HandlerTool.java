...
import com.oceanbase.obsharding_d.backend.mysql.nio.handler.builder.sqlvisitor.MysqlVisitor;
...
import com.oceanbase.obsharding_d.plan.common.item.ItemRef;
...
public final class HandlerTool {
    ...
    private static boolean isMatchSchema(String logicSchema, String table, String sourceSchema) {
        ...
        SchemaConfig schemaConfig = OBsharding_DServer.getInstance().getConfig().getSchemas().get(logicSchema);
        if (schemaConfig.isNoSharding()) {
            ShardingNode dbNode = OBsharding_DServer.getInstance().getConfig().getShardingNodes().get(schemaConfig.getDefaultSingleNode());
        ...
        if (tbConfig == null) {
            ...
            for (String shardingNode : tbConfig.getShardingNodes()) {
                ShardingNode dbNode = OBsharding_DServer.getInstance().getConfig().getShardingNodes().get(shardingNode);
