...
import static com.oceanbase.obsharding_d.net.connection.PooledConnection.*;
        ...
        if (waiting > 0) {
            if (totalConnections.incrementAndGet() <= config.getMaxCon()) {
                ...
                if (ToResolveContainer.REACH_MAX_CON.contains(alertKey)) {
                    AlertUtil.alertResolve(AlarmCode.REACH_MAX_CON, Alert.AlertLevel.WARN, "OBsharding-D", config.getId(), labels,
                ...
                AlertUtil.alert(AlarmCode.REACH_MAX_CON, Alert.AlertLevel.WARN, maxConError, "OBsharding-D", config.getId(), labels);
