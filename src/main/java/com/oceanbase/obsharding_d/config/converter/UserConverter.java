...
import com.oceanbase.obsharding_d.cluster.zkprocess.entity.user.privilege.Schema;
import com.oceanbase.obsharding_d.cluster.zkprocess.entity.user.privilege.Table;
        ...
        if (users.getVersion() != null && !Versions.CONFIG_VERSION.equals(users.getVersion())) {
            if (problemReporter != null) {
                if (Versions.checkVersion(users.getVersion())) {
                    String message = "The OBsharding-D-config-version is " + Versions.CONFIG_VERSION + ",but the " + ConfigFileName.USER_XML + " version is " + users.getVersion() + ".There may be some incompatible config between two versions, please check it";
                    problemReporter.warn(message);
                } else {
                    String message = "The OBsharding-D-config-version is " + Versions.CONFIG_VERSION + ",but the " + ConfigFileName.USER_XML + " version is " + users.getVersion() + ".There must be some incompatible config between two versions, please check it";
                    problemReporter.warn(message);
                }
            }
        }
