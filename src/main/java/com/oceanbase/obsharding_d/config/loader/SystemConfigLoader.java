...
package com.oceanbase.obsharding_d.config.loader;
...
import com.oceanbase.obsharding_d.util.SystemProperty;
...
public final class SystemConfigLoader {
    ...
    private static void postSet(SystemConfig systemConfig) {
        ...
        if (ClusterConfig.getInstance().isClusterEnable() && !systemConfig.isUseOuterHa()) {
            ...
            LOGGER.warn("when use cluster mode, you can not use simple python ha, so OBsharding-D will set useOuterHa=true to bootstrap.dynamic.cnf");
            try {
    ...
    public static void verifyOtherParam() throws IOException {
        ...
        OBsharding_DSocketOptions.getInstance().check();
    }
}
