...
public class ServerSchemaStatVisitor extends MySqlSchemaStatVisitor {
    ...
    private Column getColumnByExpr(SQLIdentifierExpr expr) {
        ...
        if (currentSchema == null) {
            ...
            for (Map.Entry<Pair<String/* schemaName */, String/* tableName */>, String/*referenceName*/> k : selectSchemaTables.entrySet()) {
                ...
                if ((schemaConfig = OBsharding_DServer.getInstance().getConfig().getSchemas().get(schemaName)) != null) {
    ...
    private void checkAliasInColumn(String tableName, boolean containSchema) {
        ...
        putAliasToMap(tableName, aliasMap.getOrDefault(tempStr, tempStr), removeApostrophe);
    }
        ...
        if (whereUnit.getSubWhereUnit().size() > 0) {
            for (WhereUnit sub : whereUnit.getSubWhereUnit()) {
                ...
                if (whereUnit.getOutRelationships().size() > 0) {
                    for (List<Condition> subConditionList : sub.getOrConditionList()) {
