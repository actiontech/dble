...
import com.oceanbase.obsharding_d.backend.mysql.nio.handler.query.impl.subquery.UpdateSubQueryHandler;
...
import com.oceanbase.obsharding_d.route.parser.druid.DruidShardingParseInfo;
import com.oceanbase.obsharding_d.route.parser.druid.RouteCalculateUnit;
...
import static com.oceanbase.obsharding_d.backend.mysql.nio.handler.query.impl.subquery.AllAnySubQueryHandler.*;
...
public final class RouterUtil {
    ...
    public static String removeSchema(String stmt, String schema) {
        return removeSchema(stmt, schema, OBsharding_DServer.getInstance().getSystemVariables().isLowerCaseTableNames());
    }
    ...
    public static RouteResultset routeFromParser(DruidParser druidParser, SchemaConfig schema, RouteResultset rrs, SQLStatement statement,
        ...
        if ((ctx.getTables() == null || ctx.getTables().size() == 0) &&
            ...
            if (schema == null) {
                schema = OBsharding_DServer.getInstance().getConfig().getSchemas().get(SchemaUtil.getRandomDb());
            }
    ...
    public static void routeNoNameTableToSingleNode(RouteResultset rrs, SchemaConfig schema) throws SQLNonTransientException {
        if (schema == null) {
            ...
            schema = OBsharding_DServer.getInstance().getConfig().getSchemas().get(db);
        }
    ...
    public static String getRandomShardingNode(Collection<String> shardingNodes) {
        ...
        Map<String, ShardingNode> shardingNodeMap = OBsharding_DServer.getInstance().getConfig().getShardingNodes();
        while (x.size() > 1) {
    ...
    private static boolean isShardingColumn(TableStat.Column column) {
        ...
        if (split.length == 2) {
            ...
            SchemaConfig schema = OBsharding_DServer.getInstance().getConfig().getSchemas().get(schemaName);
    ...
    private static RouteResultset tryRouteForTablesComplex(
        ...
        Set<String> tableSet = Sets.newHashSet();
        for (Pair<String, String> table : tables) {
            SchemaConfig schema = OBsharding_DServer.getInstance().getConfig().getSchemas().get(table.getKey());
            String tableName = table.getValue();
            tableSet.add(schema.getName() + "." + tableName);
            String testShardingNode = RouterUtil.isNoSharding(schema, tableName);
            if (testShardingNode != null && tablesRouteMap.size() == 0) {
