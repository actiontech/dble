...
import com.oceanbase.obsharding_d.server.response.ShowColumns;
import com.oceanbase.obsharding_d.server.response.ShowIndex;
import com.oceanbase.obsharding_d.server.response.ShowTableStatus;
...
public final class ServerParseShow {
    ...
    public static final int OBsharding_D_PROCESS_LIST = 14;
    ...
    private static int showDCheck(String stmt, int offset) {
        if (stmt.length() > offset++) {
            switch (stmt.charAt(offset)) {
                ...
                case 'b':
                case 'B':
                    return showOBsharding_DProcessListCheck(stmt, offset);
                default:
            }
        }
        return OTHER;
    ...
    // SHOW OBsharding-D_PROCESSLIST
    private static int showOBsharding_DProcessListCheck(String stmt, int offset) {
        ...
        if (ParseUtil.compare(stmt, offset, keyword) && !ParseUtil.isErrorTail(offset + keyword.length(), stmt)) {
            return OBsharding_D_PROCESS_LIST;
        }
        return OTHER;
    }
}
