...
package com.oceanbase.obsharding_d.server.util;
...
public final class SchemaUtil {
    ...
    public static String getRandomDb() {
        Map<String, SchemaConfig> schemaConfigMap = OBsharding_DServer.getInstance().getConfig().getSchemas();
    ...
    public static SchemaInfo getSchemaInfoWithoutCheck(String schema, String tableName) {
        ...
        SchemaConfig config = OBsharding_DServer.getInstance().getConfig().getSchemas().get(schema);
    ...
    public static boolean isNoSharding(ShardingService service, SQLSelectQuery sqlSelectQuery, SQLStatement selectStmt, SQLStatement childSelectStmt, String contextSchema, Set<String> schemas, StringPtr shardingNode)
        ...
        if (sqlSelectQuery instanceof MySqlSelectQueryBlock) {
            ...
            //CHECK IF THE SELECT LIST HAS INNER_FUNC IN,WITCH SHOULD BE DEAL BY OBsharding-D
    ...
    public static void tryAddDefaultShardingTableConfig(String schema, String tableName, String createSql, AtomicInteger tableIndex) {
        ...
        if (schemaConfig != null && (schemaConfig.getTable(tableName) == null || schemaConfig.getTable(tableName) instanceof ShardingTableFakeConfig) && schemaConfig.isDefaultShardingNode()) {
            ...
            Set<ERTable> erTables = OBsharding_DServer.getInstance().getConfig().getFuncNodeERMap().computeIfAbsent(key, k -> new HashSet<>());
            Map<ERTable, Set<ERTable>> localErRelations = OBsharding_DServer.getInstance().getConfig().getErRelations();
    ...
    public static void tryDropDefaultShardingTableConfig(String schema, String tableName) {
        ...
        if (schemaConfig != null && schemaConfig.getTable(tableName) instanceof ShardingTableFakeConfig) {
            ...
            String key = shardingTable.getFunction().getAlias() + "_" + shardingTable.getShardingNodes().toString();
            Map<String, Set<ERTable>> localFuncNodeERMap = OBsharding_DServer.getInstance().getConfig().getFuncNodeERMap();
    ...
    public static class SchemaInfo {
        ...
        public SchemaInfo(String schema, String table) {
            this.schema = schema;
            this.table = table;
            this.schemaConfig = OBsharding_DServer.getInstance().getConfig().getSchemas().get(schema);
        }
