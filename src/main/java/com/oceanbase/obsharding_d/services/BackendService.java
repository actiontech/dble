...
import com.oceanbase.obsharding_d.net.executor.ThreadContextView;
import com.oceanbase.obsharding_d.net.executor.ThreadPoolStatistic;
import com.oceanbase.obsharding_d.net.handler.BackEndCleaner;
...
import com.oceanbase.obsharding_d.net.response.ProtocolResponseHandler;
...
import com.oceanbase.obsharding_d.util.exception.UnknownTxIsolationException;
...
public abstract class BackendService extends AbstractService {
    ...
    private void handleTaskQueue(ThreadContext threadContext) {
        ...
        try {
            ...
            if (SystemConfig.getInstance().getUseThreadUsageStat() == 1) {
                workUsage = OBsharding_DServer.getInstance().getThreadUsedMap().get(threadName);
                if (threadName.contains(OBsharding_DServer.BACKEND_WORKER_NAME)) {
                    if (workUsage == null) {
            ...
            try {
                ...
                while ((task = taskQueue.poll()) != null) {
                    ...
                    try {
                        ...
                        OBsharding_DThreadPoolProvider.beginProcessBackendBusinessTask();
            ...
            // threadUsageStat end
            if (workUsage != null && threadName.contains(OBsharding_DServer.BACKEND_WORKER_NAME)) {
    ...
    private String getSetSQL(Map<String, String> usrVars, Map<String, String> sysVars, Set<String> toResetSys) {
        ...
        for (Map.Entry<String, String> entry : tmpSysVars.entrySet()) {
            String value = OBsharding_DServer.getInstance().getSystemVariables().getDefaultValue(entry.getKey());
            try {
                assert value != null;
