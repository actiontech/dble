...
public class OBsharding_DDbGroup extends ManagerWritableTable {

    public static final String TABLE_NAME = "obsharding-d_db_group";
    ...
    public int updateRows(Set<LinkedHashMap<String, String>> affectPks, LinkedHashMap<String, String> values) throws SQLException {
        ...
        RawJson dbConfig = OBsharding_DServer.getInstance().getConfig().getDbConfig();
        ...
        OBsharding_DTempConfig.getInstance().setDbConfig(dbConfig);
        return affectPks.size();
    }
    ...
    public int deleteRows(Set<LinkedHashMap<String, String>> affectPks) {
        ...
        OBsharding_DTempConfig.getInstance().setDbConfig(dbGroupJson);
        return affectPks.size();
    }
    ...
    private void checkDeleteRule(Set<LinkedHashMap<String, String>> affectPks) {
        for (LinkedHashMap<String, String> affectPk : affectPks) {
            //check user-group
            OBsharding_DRwSplitEntry obsharding_dRwSplitEntry = (OBsharding_DRwSplitEntry) ManagerSchemaInfo.getInstance().getTables().get(OBsharding_DRwSplitEntry.TABLE_NAME);
            boolean existUser = obsharding_dRwSplitEntry.getRows().stream().anyMatch(entry -> entry.get(OBsharding_DRwSplitEntry.COLUMN_DB_GROUP).equals(affectPk.get(COLUMN_NAME)));
            if (existUser) {
                throw new ConfigException("Cannot delete or update a parent row: a foreign key constraint fails `obsharding-d_rw_split_entry`(`db_group`) REFERENCES `obsharding-d_db_group`(`name`)");
            }
            //check instance-group
            OBsharding_DDbInstance obsharding_dDbInstance = (OBsharding_DDbInstance) ManagerSchemaInfo.getInstance().getTables().get(OBsharding_DDbInstance.TABLE_NAME);
            boolean existInstance = obsharding_dDbInstance.getRows().stream().anyMatch(entry -> entry.get(OBsharding_DDbInstance.COLUMN_DB_GROUP).equals(affectPk.get(COLUMN_NAME)));
            if (existInstance) {
                throw new ConfigException("Cannot delete or update a parent row: a foreign key constraint fails `obsharding-d_db_instance`(`db_group`) REFERENCES `obsharding-d_db_group`(`name`)");
            }
            //check sharding_node-group
            OBsharding_DShardingNode obsharding_dShardingNode = (OBsharding_DShardingNode) ManagerSchemaInfo.getInstance().getTables().get(OBsharding_DShardingNode.TABLE_NAME);
            boolean existShardingNode = obsharding_dShardingNode.getRows().stream().anyMatch(entry -> entry.get(OBsharding_DShardingNode.COLUMN_DB_GROUP).equals(affectPk.get(COLUMN_NAME)));
            if (existShardingNode) {
                throw new ConfigException("Cannot delete or update a parent row: a foreign key constraint fails `obsharding-d_sharding_node`(`db_group`) REFERENCES `obsharding-d_db_group`(`name`)");
            }
        }
    }
    ...
    public void updateTempConfig() {
        OBsharding_DTempConfig.getInstance().setDbConfig(OBsharding_DServer.getInstance().getConfig().getDbConfig());
    }
}
