...
public class OBsharding_DDbGroup extends ManagerWritableTable {
    ...
    public static final String TABLE_NAME = "obsharding_d_db_group";
    ...
    private void checkDeleteRule(Set<LinkedHashMap<String, String>> affectPks) {
        for (LinkedHashMap<String, String> affectPk : affectPks) {
            ...
            if (existUser) {
                throw new ConfigException("Cannot delete or update a parent row: a foreign key constraint fails `obsharding_d_rw_split_entry`(`db_group`) REFERENCES `obsharding_d_db_group`(`name`)");
            }
            ...
            if (existInstance) {
                throw new ConfigException("Cannot delete or update a parent row: a foreign key constraint fails `obsharding_d_db_instance`(`db_group`) REFERENCES `obsharding_d_db_group`(`name`)");
            }
            ...
            if (existShardingNode) {
                throw new ConfigException("Cannot delete or update a parent row: a foreign key constraint fails `obsharding_d_sharding_node`(`db_group`) REFERENCES `obsharding_d_db_group`(`name`)");
            }
        }
    }
