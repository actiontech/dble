...
public class OBsharding_DDbInstance extends ManagerWritableTable {

    public static final String TABLE_NAME = "obsharding_d_db_instance";
    ...
    public int insertRows(List<LinkedHashMap<String, String>> rows) throws SQLException {
        ...
        OBsharding_DDbGroup obsharding_dDbGroup = (OBsharding_DDbGroup) ManagerSchemaInfo.getInstance().getTables().get(OBsharding_DDbGroup.TABLE_NAME);
        List<LinkedHashMap<String, String>> tempDbGroupMapList = obsharding_dDbGroup.getTempRowList();
        List<DBGroup> tempDbGroupList = tempDbGroupMapList.stream().map(obsharding_dDbGroup::transformRowToDBGroup).collect(Collectors.toList());
    ...
    public void afterExecute() {
        ...
        for (DBGroup dbGroup : dbGroups.getDbGroup()) {
            obsharding_dDbGroup.getTempRowList().removeIf(group -> StringUtil.equals(group.get(OBsharding_DDbGroup.COLUMN_NAME), dbGroup.getName()));
        }
    }
