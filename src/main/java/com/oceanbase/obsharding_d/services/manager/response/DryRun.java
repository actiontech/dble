...
import com.oceanbase.obsharding_d.config.*;
...
import com.oceanbase.obsharding_d.meta.table.DryRunGetNodeTablesHandler;
...
public final class DryRun {
    ...
    public static void execute(ManagerService service) {
        try {
            ...
            try {
                LOGGER.info("[dry-run] check OBsharding-D and mysql version start");
                //check version
                ConfigUtil.checkOBsharding_DAndMysqlVersion(loader.getDbGroups());
            } catch (Exception e) {
                LOGGER.debug("[dry-run] check OBsharding-D and mysql version, catch exception:", e);
                list.add(new ErrorInfo("Backend", "ERROR", "check OBsharding-D and mysql version exception: " + e.getMessage()));
            }
            ...
            if (!ClusterConfig.getInstance().isClusterEnable()) {
                list.add(new ErrorInfo("Cluster", "NOTICE", "OBsharding-D is in single mod"));
            }
    ...
    private static void ucoreConnectionTest(List<ErrorInfo> list) {
        try {
            ...
            list.add(new ErrorInfo("Cluster", "ERROR", "OBsharding-D in cluster but all the ucore can't connect"));
        }
    }
