...
import com.oceanbase.obsharding_d.route.parser.ManagerParseConfig;
...
import com.oceanbase.obsharding_d.singleton.CronScheduler;
...
import static com.oceanbase.obsharding_d.services.manager.response.ChangeItemType.PHYSICAL_DB_INSTANCE;
...
public final class ReloadConfig {
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    //step 4 write the reload flag and self reload result into cluster center,notify the other OBsharding-D to reload
                    ...
                    //step 5 start a loop to check if all the OBsharding-D in cluster is reload finished
    ...
    public static ReloadResult reloadByConfig(final int loadAllMode, boolean isWriteToLocal) throws Exception {
        RawJson userConfig = OBsharding_DTempConfig.getInstance().getUserConfig();
        userConfig = userConfig == null ? OBsharding_DServer.getInstance().getConfig().getUserConfig() : userConfig;
        RawJson dbConfig = OBsharding_DTempConfig.getInstance().getDbConfig();
        dbConfig = dbConfig == null ? OBsharding_DServer.getInstance().getConfig().getDbConfig() : dbConfig;
        RawJson shardingConfig = OBsharding_DTempConfig.getInstance().getShardingConfig();
        shardingConfig = shardingConfig == null ? OBsharding_DServer.getInstance().getConfig().getShardingConfig() : shardingConfig;
        RawJson sequenceConfig = OBsharding_DTempConfig.getInstance().getSequenceConfig();
        sequenceConfig = sequenceConfig == null ? OBsharding_DServer.getInstance().getConfig().getSequenceConfig() : sequenceConfig;
        ...
        OBsharding_DServer.getInstance().getConfig().syncJsonToLocal(isWriteToLocal);
        return reloadResult;
    }
    ...
    private static ReloadResult reload(final int loadAllMode, RawJson userConfig, RawJson dbConfig, RawJson shardingConfig, RawJson sequenceConfig) throws Exception {
        ...
        try {
            ...
            ServerConfig oldConfig = OBsharding_DServer.getInstance().getConfig();
            boolean result;
            try {
    ...
    private static SystemVariables checkVersionAGetSystemVariables(ConfigInitializer loader, Map<String, PhysicalDbGroup> newDbGroups, List<ChangeItem> changeItemList, boolean forceAllReload) throws Exception {
        ...
        if (forceAllReload || ConfigUtil.isAllDbInstancesChange(changeItemList)) {
            //check version
            ConfigUtil.checkOBsharding_DAndMysqlVersion(newDbGroups);
            ...
            //check version
            ConfigUtil.checkOBsharding_DAndMysqlVersion(changeItemList, loader);
            //check packetSize/lowerCase
     ...
     * memory:OBsharding-D_information/cluster synchronization
     */
    ...
    private static void syncShardingNode(ConfigInitializer loader) {
        Map<String, ShardingNode> oldShardingNodeMap = OBsharding_DServer.getInstance().getConfig().getShardingNodes();
    ...
    private static List<ChangeItem> differentiateChanges(ConfigInitializer newLoader) {
        ...
        //user
        //old
        ServerConfig oldServerConfig = OBsharding_DServer.getInstance().getConfig();
    ...
    private static SystemVariables getSystemVariablesFromdbGroup(ConfigInitializer loader, Map<String, PhysicalDbGroup> newDbGroups) throws Exception {
        ...
        if (newSystemVariables == null) {
            if (loader.isFullyConfigured()) {
                ...
                newSystemVariables = OBsharding_DServer.getInstance().getSystemVariables();
            }
        }
        return newSystemVariables;
    }
