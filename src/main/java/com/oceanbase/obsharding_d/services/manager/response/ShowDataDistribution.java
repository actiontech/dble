...
public final class ShowDataDistribution {
    ...
    public static void execute(ManagerService service, String name) {
        ...
        SchemaConfig schemaConfig = OBsharding_DServer.getInstance().getConfig().getSchemas().get(schemaInfo[0]);
        ...
        if (!succeed.get()) {
            service.writeErrMessage(ErrorCode.ER_YES, "occur Exception, so see OBsharding-D.log to check reason");
            return;
        }
