...
import static com.oceanbase.obsharding_d.cluster.ClusterController.CONFIG_MODE_ZK;
...
public final class SystemParams {
    ...
    private SystemParams() {
        ...
        if (ClusterConfig.getInstance().isClusterEnable()) {
            readOnlyParams.add(new ParamInfo("clusterMode", ClusterConfig.getInstance().getClusterMode(), "ClusterMode for OBsharding-D cluster center"));
            readOnlyParams.add(new ParamInfo("clusterIP", ClusterConfig.getInstance().getClusterIP(), "OBsharding-D cluster center address, If clusterMode is zk, it is a full address with port"));
            if (!ClusterConfig.getInstance().getClusterMode().equalsIgnoreCase(CONFIG_MODE_ZK)) {
                readOnlyParams.add(new ParamInfo("clusterPort", ClusterConfig.getInstance().getClusterPort() == 0 ? "" : ClusterConfig.getInstance().getClusterPort() + "", "OBsharding-D cluster center address's port"));
            ...
            readOnlyParams.add(new ParamInfo("rootPath", ClusterConfig.getInstance().getRootPath(), "OBsharding-D cluster center's root path, the default value is /OBsharding-D"));
            readOnlyParams.add(new ParamInfo("clusterId", ClusterConfig.getInstance().getClusterId(), "OBsharding-D cluster Id"));
            readOnlyParams.add(new ParamInfo("needHaSync", ClusterConfig.getInstance().isNeedSyncHa() + "", "Whether OBsharding-D use cluster ha, The default value is false"));
        }
        ...
        readOnlyParams.add(new ParamInfo("serverId", sysConfig.getServerId() + "", "ServerID of machine which install OBsharding-D, the default value is the machine IP"));
        ...
        readOnlyParams.add(new ParamInfo("district", sysConfig.getDistrict() + "", "The location of the OBsharding-D"));
        readOnlyParams.add(new ParamInfo("dataCenter", sysConfig.getDataCenter() + "", "The data center where the OBsharding-D resides"));
